...
export const HomeContent = () => {
  const { data: sessionsRes, isLoading: sessionsLoading, isError: sessionsError } =
    useGetSessionsQuery()
  const { data: coursesRes, isLoading: coursesLoading, isError: coursesError } =
    useGetCoursesQuery()
  const { data: blogsRes, isLoading: blogsLoading, isError: blogsError } = useGetBlogsQuery({
    limit: 3,
  })

  const sessions = sessionsRes?.data?.sessions || []
  const courses = (coursesRes?.data?.courses || []).slice(0, 3)
